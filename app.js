...
function getWeekDates(dateStr) {
  ...
  mon.setDate(dt.getDate() - ((dt.getDay() + 6) % 7));  // Monday start
...
function calcScore(dateStr) {
  ...
  return denom > 0
  ? Math.round((habitsDone + tasksDone + pinnedDone) / denom * 100)
  : 0;
}
...
function openEmojiPicker() {
  ...
  .map(e => `<button class="emoji-opt" data-emoji="${e}">${e}</button>`)
  .join('');
...
function updateScoreStrip() {
  ...
  const sv  = parseFuzzyTime(d.slept);
  const wv  = parseFuzzyTime(d.woke);
  let sleep = '—';
  if (sv !== null && wv !== null) {
    let m = wv - sv;
    if (m < 0) m += 1440;
...
function renderWeek() {
  ...
  document.getElementById('week-grid').innerHTML = dates.map((d, i) => {
    ...
    `<div class="wdc-dot ${data?.habits?.[h.id] ? 'done' : ''}"></div>`
    ).join('');

    return `
    <div class="week-day-card ${isToday ? 'today-card' : ''} ${isCur ? 'selected-card' : ''}"
  ...
  ? Math.max(...habits.map(h => calcStreak(h.id)))
  : 0;
...
function renderTrends() {
  ...
  const sleepHours = days.map(d => {
    ...
    const w = parseFuzzyTime(data.woke);
...
function updateNotifStatus() {
  ...
  if (Notification.permission === 'granted') {
    ...
    el.textContent = 'Toggle on to enable notifications';
...
function scheduleReminders() {
  remTimers.forEach(clearTimeout);
  remTimers = [];
  ...
  function schedule(title, body, timeStr, enabled) {
    ...
    const t = setTimeout(() => {
      ...
      schedule(title, body, timeStr, true);  // re-schedule for tomorrow
    }, msUntil(timeStr));
    remTimers.push(t);
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Tasks / habits / pinned — add buttons + enter key
  document.getElementById('add-task-btn').addEventListener('click', addTask);
  document.getElementById('add-task-inp').addEventListener('keydown', e => {
    if (e.key === 'Enter') addTask();
  });

    document.getElementById('add-habit-btn').addEventListener('click', addHabit);
    document.getElementById('add-habit-inp').addEventListener('keydown', e => {
      if (e.key === 'Enter') addHabit();
    });

      document.getElementById('add-pin-btn').addEventListener('click', addPinnedTask);
      document.getElementById('add-pin-inp').addEventListener('keydown', e => {
        if (e.key === 'Enter') addPinnedTask();
      });
        ...
        document.getElementById('emoji-popup').addEventListener('click', e => {
          ...
          if (btn) {
            ...
            closeEmojiPicker();
          } else if (e.target === document.getElementById('emoji-popup')) {
            closeEmojiPicker();
          }
        });
